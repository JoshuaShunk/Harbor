/// CLI symlink installation is only supported on Unix (macOS/Linux).
/// On Windows, the bundled CLI is available via the app's resource directory.
#[cfg(unix)]
mod unix {
...
#[cfg(unix)]
pub use unix::install_cli_symlink;

#[cfg(not(unix))]
pub fn install_cli_symlink(_handle: &tauri::AppHandle) {
    // No-op on Windows — CLI symlink not applicable
}
